			...
			<form name="form1" id="form1" method="get">
				...
				<div>
					...
					<c:when test="${sessionScope.c.c_id == dto.bc_id}">
						<input name="b_title" id="title" size="80" value="${dto.b_title}" placeholder="제목을 입력해주세요">
					</c:when>
					<c:otherwise>
						${dto.b_title}
					</c:otherwise>
				...
				<div>
					...
					<c:choose>
					<c:when test="${sessionScope.c.c_id == dto.bc_id}">
						<textarea name="b_content" id="content" rows="4" cols="80" placeholder="내용을 입력해주세요">${dto.b_content}</textarea>
					</c:when>
					<c:otherwise>
						${dto.b_content}
					</c:otherwise>
